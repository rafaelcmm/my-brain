...
4. CI and release automation templates:
   lint/test/compose validation, release-please, GHCR publish.
5. Model-invoked Claude skills and curator agent templates under .claude/.
...
1. MYBRAIN_MODE: memory or full.
2. MYBRAIN_LLM_MODEL: local model used in full mode.
3. MYBRAIN_BIND_HOST: keep 127.0.0.1 for local-only safety.
4. MYBRAIN_DB_PASSWORD: generated by installer if placeholder is present.
5. MYBRAIN_AUTH_TOKEN_FILE: token file path mounted into gateway.
...
Current release baseline is bootstrap-quality and designed for local-first deployments. Team/internet exposure requires TLS, stronger auth model, and rate limiting.
