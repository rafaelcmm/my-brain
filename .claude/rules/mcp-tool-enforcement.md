...
1. Context enrichment for non-trivial requests
   ...
   - Required order: `hooks_capabilities` -> `mb_context_probe` -> `mb_recall`
...
6. Periodic quality maintenance
   ...
   - Required order: `hooks_capabilities` -> `hooks_stats` -> `mb_digest` -> `mb_recall`

## Degraded-Mode Enforcement

When `hooks_capabilities` reports `engine=false`:
...
1. Good: Non-trivial request triggers `my-brain-context`, calls `hooks_capabilities` first, then probes context, then scoped recall.
